...
export const requireSession = async (req: Request, res: Response, next: NextFunction) => {
    ...
    // Use originalUrl to be safe against router mounting
    // Use startsWith for stricter matching
    if (req.originalUrl.startsWith('/auth/register') || req.originalUrl.startsWith('/auth/login')) {
        return next();
    }
    ...
    if (!authHeader?.startsWith('Bearer ')) {
        res.status(401).json({
            error: 'UNIQUE_ERROR_STRING_NO_TOKEN',
            path: req.path,
            method: req.method,
    ...
    // Fix Issue 3: Enforce 'ACTIVE' status
    if (session.active_workspace_id && session.active_workspace) {
        if (session.active_workspace.status === 'ACTIVE') {
            ...
            // We do NOT set req.activeWorkspaceId, effectively "deselecting" it for this request.
            // If the route requires workspace, requireWorkspace middleware will catch it (409).
        }
    }
